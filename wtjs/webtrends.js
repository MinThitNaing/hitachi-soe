...
// Created: 15:47 2013/01/17
// Modified: 2013/12/24, 2014/12/17, 2015/02/12, 2015/06/17, 2018/10/03, 2018/10/15

function WebTrends(){
	var that=this;
	// begin: user modifiable
	this.dcsid="dcshyjdor0000000wocvyzq5r_2m1y";
	this.domain="statse.webtrendslive.com";
	this.timezone=-5;
	this.fpcdom=".hitachi.com";
	this.onsitedoms="www.hitachi.com, www2.hitachi.com, search.hitachi.co.jp, www8.hitachi.co.jp, hitachi.com";
	this.micsitedirs="csr, csr/sc, environment, hitachitree, procurement, rd, society, products/it, products/it/network, products/it/software, hirt, IR-e, rev, products/smartcity, advertising/people, businesses/innovation, businesses/infrastructure, businesses/healthcare, businesses/elevator, products/power/hpl, products/power/PBT, products/power/solar-power, products/power/TandD, products/power/wind-turbine, rd/design, sustainability";
	this.onsitepths="";
	this.offsitepths="";
	this.downloadtypes="xls,doc,pdf,txt,csv,zip,docx,xlsx,mp4";
	this.navigationtag="table,div";
	...
	this.re=(function(){return(window.RegExp?(that.i18n?{"%25":/\%/g,"%26":/\&/g,"%23":/\#/g}:{"%09":/\t/g,"%20":/ /g,"%23":/\#/g,"%26":/\&/g,"%2B":/\+/g,"%3F":/\?/g,"%5C":/\\/g,"%22":/\"/g,"%7F":/\x7F/g,"%A0":/\xA0/g}):"");})();
}
...
WebTrends.prototype.dcsIsOnsite3=function(host,e){
	var furi=window.location.pathname; 
	var micdir=''; 
	var len=0;
	var pth=e.pathname?((e.pathname.indexOf("/")!=0)?"/"+e.pathname:e.pathname):"/";  //target path - IE drops the leading '/'
	...
	if (host.length>0){
		...
		else if (typeof(this.onsitedoms.test)=="function"){
		...
		var lenm=this.micsitedirs.length;
		if(lenm>0){ 
			var dirs=this.dcsSplit(this.micsitedirs);
			var len=dirs.length;
		}
		if (typeof(this.micsitedirs)=="function"){
			return this.micsitedirs.test(e);
		} else if(lenm>0){
			for (var i=len-1;i>=0;i--){
				if ( furi.toLowerCase().search( '/'+dirs[i].toLowerCase()+'' )==0 ) {
					micdir='/'+dirs[i].toLowerCase()+'';					
					if( pth.search( micdir )==0 ) {
						for(var j=len-1;j>=0;j--) {						
							var dirj=dirs[j];
							if( micdir.search('/'+dirj.toLowerCase()) && pth.toLowerCase().search( '/'+dirj.toLowerCase() )==0) { 							
								return false;
							}							
						}
						return true;
		...
		for(var i=0;i<len;i++){	
			if ( pth.toLowerCase().search( '/'+dirs[i].toLowerCase() )==0 ) {
				return false;
			}
		}


		var c=furi; //window.location.pathname;
		var t=pth; //e.pathname;
		var t_t=0;
		var onp=this.dcsSplit(this.onsitepths);
		var ofp=this.dcsSplit(this.offsitepths); 				

		if(t.indexOf('/',1)==-1) {			
			return true;
		}
		if (this.onsitepths.length==0) {
						//+++			
						if( this.offsitepths.length==0 ) {
							return true;
						} else {
							if (ofp.length>0) {
								for(var k=0;k<ofp.length;k++) {	//LOOP #2
									if(t.indexOf(ofp[k])>-1 && c.indexOf(ofp[k])==-1) {
										return false; //BEACON
									} else { /*t_t=0; MOVE ON NEXT */ }
								}
							
							} else { /*t_t=0; /*/ }
							return true; //NO BEACON
						}
						//+++

			return true; //NO BEACON
		} else {
		  	//�I���T�C�g�p�X�ݒ�L�� && �^�[�Q�b�g���g�b�v�łȂ�
			if (onp.length>0) {
				for(var j=0;j<onp.length;j++) {	//LOOP #1
					if (t.indexOf(onp[j])>-1) {   	//�^�[�Q�b�g�̓I���T�C�g����
				...
				//�^�[�Q�b�g�̓I�t�T�C�g�B�������A���^�[�Q�b�g�Ɠ����f�B���N�g���ɂ���ꍇ�̓I���T�C�g����
				if(''+t.match(/\/[a-z0-9A-Z]+\//)==''+c.match(/\/[a-z0-9A-Z]+\//)) {
					return true; //NO BEACON
				} else {
					return false; //BEACON
				} 	
			}
			//return true; //NO BEACON - default
		}
	} //host
	return false; //BEACON - default
}
...
WebTrends.prototype.dcsDownload=function(evt){
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href){
		    ...
		    if (this.dcsIsOnsite3(hn,e)&&this.dcsTypeMatch(e.pathname,this.downloadtypes)){
			...
			setTimeout(Sleep, 0); 	//Added on 2010-08-30 for FireFox
...
WebTrends.prototype.dcsOffsite=function(evt){
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href){
		    var hn=e.hostname?(e.hostname.split(":")[0]):"";
		    var pr=e.protocol||"";
		    if ((hn.length>0)&&(pr.indexOf("http")==0)&&!this.dcsIsOnsite3(hn,e)){
			    var qry=e.search?e.search.substring(e.search.indexOf("?")+1,e.search.length):"";
			    var pth=e.pathname?((e.pathname.indexOf("/")!=0)?"/"+e.pathname:e.pathname):"/";
			    setTimeout(Sleep, 0);
...
WebTrends.prototype.dcsAnchor=function(evt){
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href){
		    var hn=e.hostname?(e.hostname.split(":")[0]):"";
		    if (this.dcsIsOnsite3(hn,e)&&e.hash&&(e.hash!="")&&(e.hash!="#")){
		        var qry=e.search?e.search.substring(e.search.indexOf("?")+1,e.search.length):"";
			    var pth=e.pathname?((e.pathname.indexOf("/")!=0)?"/"+e.pathname:e.pathname):"/";
			    setTimeout(Sleep, 0);
...
function Sleep() {
    ...
    while( (tStart + 500) > Number(new Date()) ) {};
}